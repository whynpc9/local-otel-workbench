...
The workbench maps AI SDK fields such as `ai.prompt.messages`, `ai.prompt.tools`, `ai.response.text`, `ai.response.toolCalls`, `ai.usage.*`, `ai.response.finishReason`, and `ai.response.providerMetadata` into the GenAI view. See the repository `docs/genai-framework-support.md` for the framework matrix and validation probes.
